...
//                }
                ...
                String data = "WHOAMI|" + nativeMac.toUpperCase().replace(":", "-") + "|" + nativeIp
                        + "/" + nativeNetmask + "/" + nativeGateway + "|TH-S6100-V2.0";
    ...
    class UpdateNativeIpThread extends Thread {
        ...
        public void run() {
            ...
            String a = nativeMac.toUpperCase().replace(":", "-");
            if (serverMac.equals(a)) {
                //设置静态ip
                if (netWorkType == 1)
                 aa  = App.getSystemManager().ZYsetEthStaticMode(serverIp, serverGateway, serverNetmask, AppConfig.DNS, AppConfig.DNS);
                else
                 aa =   App.getSystemManager().ZYsetWifiStaticMode(serverIp, serverGateway, serverNetmask, AppConfig.DNS, AppConfig.DNS);
                Logutil.i("哈哈，已修改了Ip"+aa);
                App.startSpeaking(UIUtils.getString(R.string.str_ip_update_success));
            }
        }
    }
    ...
    class RebootThread extends Thread {
        ...
        public void run() {
            if (serverMac.equals(nativeMac.toUpperCase().replace(":", "-"))) {
                ...
                //重启应用
                Intent intent = App.getApplication().getPackageManager().getLaunchIntentForPackage("com.tehike.client.stc.app.project");
                intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                startActivity(intent);
                App.startSpeaking(UIUtils.getString(R.string.str_device_restart_success));
            }
        }
    }
}
