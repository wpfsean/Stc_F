...
public class Linphone {
    ...
    public static void addCallback(RegistrationCallback registrationCallback,
        ...
        if (SipService.isReady()) {
            SipService.addRegistrationCallback(registrationCallback);
            SipService.addPhoneCallback(phoneCallback);
        } else {
    ...
    public static void callTo(String num, boolean isVideoCall) {
        ...
        if (!num.equals("")) {
            ...
            SipUtils.getInstance().startSingleCallingTo(phone, isVideoCall);
        }
    }
    ...
    public static void toggleMicro(boolean isMicMuted) {
        SipUtils.getInstance().toggleMicro(isMicMuted);
    }
    ...
    public static void toggleSpeaker(boolean isSpeakerEnabled) {
        SipUtils.getInstance().toggleSpeaker(isSpeakerEnabled);
    }
    ...
    private static class ServiceWaitThread extends Thread {
        ...
        public void run() {
            ...
            SipService.addPhoneCallback(mPhoneCallback);
            SipService.addRegistrationCallback(mRegistrationCallback);
    ...
    private static void loginToServer() {
        try {
            ...
            SipUtils.getInstance().registerUserAuth(mUsername, mPassword, mServerIP);
        } catch (LinphoneCoreException e) {
            e.printStackTrace();
        }
    }
    ...
    public static boolean getVideoEnabled() {
        LinphoneCallParams remoteParams = SipManager.getLc().getCurrentCall().getRemoteParams();
    ...
    public static void onResume() {
        ...
        if (mAndroidVideoWindow != null) {
            synchronized (mAndroidVideoWindow) {
                SipManager.getLc().setVideoWindow(mAndroidVideoWindow);
            }
        }
    }
    ...
    public static void onPause() {
        if (mAndroidVideoWindow != null) {
            synchronized (mAndroidVideoWindow) {
                SipManager.getLc().setVideoWindow(null);
            }
        }
    ...
    public static void fixZOrder(SurfaceView rendering, SurfaceView preview) {
    ...
    private static void setVideoWindow(Object o) {
        SipManager.getLc().setVideoWindow(o);
    }

    private static void removeVideoWindow() {
        LinphoneCore linphoneCore = SipManager.getLc();
    ...
    private static void setPreviewWindow(Object o) {
        SipManager.getLc().setPreviewWindow(o);
    }

    private static void removePreviewWindow() {
        SipManager.getLc().setPreviewWindow(null);
    }
